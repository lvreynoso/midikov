...
const transformMIDI = midi => {
  ...
  for (let index = 0; index < midi.tracks.length; index++) {
    ...
    let newTrackEvents = trackEvents.map(event => {
      ...
      alpha += event.delta; // console.log(alpha);
      ...
      if (event.type == _midievents.default.EVENT_MIDI && event.subtype == _midievents.default.EVENT_MIDI_PROGRAM_CHANGE) {
        return event;
      } else if (event.type == _midievents.default.EVENT_MIDI && (event.subtype == _midievents.default.EVENT_MIDI_NOTE_ON || event.subtype == _midievents.default.EVENT_MIDI_NOTE_OFF)) {
  ...
  tracks.forEach(track => {
    if (trackNotes[track].length == 0) {
      delete trackNotes[track]; // console.log(`Deleted track ${track}`);
    }
  });
  let midiData = {
...
var _default = transformMIDI;
exports.default = _default;
//# sourceMappingURL=transform-midi.js.map