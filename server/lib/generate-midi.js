...
const generate = (markovData, order, category) => {
  ...
  let keySignature = {
    ...
    scale: parseInt(keyScaleArray[1], 10) // console.log(keyScaleArray);
  ...
  let timeSignature = {
    ...
    param4: timeDataArray[3] // timeSignature.data = timeDataArray;
  ...
  let tempo = {
    ...
    subtype: 0x51,
    length: 0x03,
    tempo: parseInt(codedTempo, 10) // tempo.tempo = parseInt(codedTempo, 10);

  };
  metaData.tempo = tempo; // console.log(metaData);
  // that finishes the metadata

  let tracks = {}; // now we generate each track
...
function generateTrack(noteMap, markovOrder, distance) {
  ...
  for (let step = 0; step <= distance; step++) {
    ...
    let note = {
      ...
      duration: parseInt(decodedNote[3], 10) // note.pitch = parseInt(decodedNote[0], 10)
